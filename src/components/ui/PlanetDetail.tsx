...
export default function PlanetDetail({ planet, date, onClose }: PlanetDetailProps) {
  ...
  return (
    <>
      ...
      <div className={[
        // Mobile: fixed bottom sheet above TimeControls
        'fixed bottom-11 left-0 right-0 z-50',
        'max-h-[65vh] overflow-y-auto',
        'rounded-t-2xl',
        // Desktop: absolute top-left panel
        'md:absolute md:top-4 md:left-4',
        'md:bottom-auto md:right-auto',
        'md:w-64 md:max-h-none md:overflow-visible',
        'md:rounded-lg',
        // Shared
        'bg-black/90 backdrop-blur-sm border border-white/10 p-4 text-sm',
      ].join(' ')}>
        {/* Mobile handle */}
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={onClose} className="text-white/40 hover:text-white text-lg p-1">
            &times;
          </button>
        </div>
